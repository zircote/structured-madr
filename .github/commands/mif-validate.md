---
description: Validate every ADR's MIF projection against the level in .github/config.yml (fail-closed at the level).
---

Run the MIF conformance gate over the repository's ADRs.

1. Read `.github/config.yml` for `mifConformanceLevel` and `adrPath`.
2. Run: `npm run validate:mif` (which invokes `node .github/bin/mif-validate.js`).
